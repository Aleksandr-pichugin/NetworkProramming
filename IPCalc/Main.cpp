...
BOOL CALLBACK DlgProc(HWND hwnd, UINT uMsg, WPARAM wParam, LPARAM lParam)
{
	...
	case WM_INITDIALOG:
	{
		HWND hSpin = GetDlgItem(hwnd, IDC_SPIN_PREFICS);
	...
	case WM_NOTIFY:
	{
		//Здесь лучше отслеживать изменения префикса , а изменение маски лучше отслеживать 'WM_COMMAND'.
...
VOID PrintInfo(HWND hwnd)
{
	...
	//DWORD dwBroadcas = dwIPaddress & dwIPmask;
	sprintf(sz_NetworkIP_buffer, "Адрес сети:\t\t\t%s", IPaddressToString(dwIPaddress & dwIPmask, sz_buffer));
	sprintf(sz_BroadcastIP_buffer, "Широковещательный сети:\t%s", IPaddressToString(dwIPaddress | ~dwIPmask, sz_buffer));
